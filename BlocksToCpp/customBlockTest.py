...
def translate(file, testDir, outputFile, cDir):
	...
	proc = subprocess.Popen(["python", "blocklyTranslatorCustom.py", "-x", testDir + file], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def checkSyntax(file, testDir, outputFile, cDir):
	...
	exitCode = subprocess.call(["g++", "Motor.o", cfile, "-std=gnu++11"])
...
def setupTest(typeName):
	...
	subprocess.call(["cp", "Motor.cpp", "cCode/customFiles/Motor.cpp"])
	subprocess.call(["cp", "Motor.o", "cCode/customFiles/Motor.o"])
	subprocess.call(["cp", "Motor.h", "cCode/customFiles/Motor.h"])
